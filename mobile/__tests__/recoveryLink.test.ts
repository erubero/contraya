...
describe('parseRecoveryParams', () => {
  it('reads the session out of the fragment (implicit flow, the default)', () => {
    const p = parseRecoveryParams(
      'warraya://reset-password#access_token=abc.def.ghi&refresh_token=rrr&expires_in=3600&token_type=bearer&type=recovery'
    );
  ...
  it('reads the code out of the query string (pkce flow)', () => {
    const p = parseRecoveryParams('warraya://reset-password?code=some-auth-code');
  ...
  it('surfaces an expired-link error from the fragment', () => {
    const p = parseRecoveryParams(
      'warraya://reset-password#error=access_denied&error_code=otp_expired&error_description=Email+link+is+invalid+or+has+expired'
    );
  ...
  it('surfaces an error from the query string too', () => {
    const p = parseRecoveryParams('warraya://reset-password?error=access_denied');
    expect(p.error).toBe('access_denied');
  });

  it('prefers the human-readable description over the error code', () => {
    const p = parseRecoveryParams(
      'warraya://reset-password#error=access_denied&error_description=Something+went+wrong'
    );
    expect(p.error).toBe('Something went wrong');
  });

  it('returns all-null for a bare link with nothing attached', () => {
    const p = parseRecoveryParams('warraya://reset-password');
  ...
  it('handles the triple-slash form expo-linking can produce', () => {
    const p = parseRecoveryParams('warraya:///reset-password#access_token=t&refresh_token=r');
