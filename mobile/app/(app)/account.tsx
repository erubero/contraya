...
// button that erases all of it. Deletion routes to its own confirmation
// screen (delete-account.tsx) so the consequences get real room and the
// destructive tap is never a mis-tap away from the theme switcher.
export default function Account() {
  const theme = useTheme();
  const router = useRouter();
  const { email, userId, displayName, avatarPath, updateProfile } = useAuth();
  ...
  return (
    ...
    >
      ...
      <View style={[settingsCard(theme), { padding: 16, alignItems: 'center', gap: 12 }]}>
        ...
        <Text style={{ color: theme.mutedForeground, fontSize: 13 }} numberOfLines={1}>
          {email ?? 'Not signed in'}
        </Text>
      </View>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
          <View style={{ flex: 1 }}>
            <TextInput
              ...
              style={{ borderColor: theme.border, borderWidth: 1, borderRadius: RADIUS, padding: 13, paddingRight: 38, fontSize: 15, color: theme.foreground, backgroundColor: theme.card }}
            />
            {!nameDirty && !savingName && (
              ...
              >
                <Ionicons name="pencil" size={16} color={theme.mutedForeground} />
              </View>
            )}
          </View>
          {(nameDirty || savingName) && (
